<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $sql_cidade = "SELECT id FROM cidade WHERE nome_cidade = '$cidade'";
        $result_cidade = $conn->query($sql_cidade);

        if ($result_cidade->num_rows > 0) {
            $row_cidade = $result_cidade->fetch_assoc();
            $id_cidade = $row_cidade['id'];

            $sql_pessoa = "INSERT INTO pessoa (nome, idade, data_nasc, id_cidade)
                       VALUES ('$nome', '$idade', '$data_nasc', '$id_cidade')";

            if ($conn->query($sql_pessoa) === TRUE) {
                echo "Cadastro de pessoa realizado com sucesso!";
            } else {
